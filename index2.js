!function () {
    metadata = {
        systemName: "Regulatory.ATP.Integration.MuleSoft.FileAPI.Test2",
        displayName: "Regulatory ATP MuleSoft File API Test 2",
    ...
    onexecute = async function ({
        ...
        switch (n) {
        case "atpFile":
            await async function (n, a, s, o) {
                switch (n) {
                case "get":
                    await function (t, n, a) {
                        return new Promise((t, s) => {
                            var o = new XMLHttpRequest;
                            o.onreadystatechange = function () {
                                try {
                                    if (console.log("xhr.readyState: " + o.readyState), 4 !== o.readyState)
                                        return;
					...
					var reader = new FileReader();
                                    ...
                                    postResult({
                                        ...
                                        size: o.getResponseHeader("Content-Length")
                                    }),
                                    t()
                                } catch (e) {
                                    s(e)
                            ...
                            var i = a.MuleSoftBaseURL + e + n.fileName;
                    ...
                    break;
                case "put":
                    await function (e, n, a) {
                        return new Promise((e, s) => {
                            (l = new XMLHttpRequest).onreadystatechange = function () {
                                try {
                                    if (console.log("xhr.readyState: " + l.readyState), 4 !== l.readyState)
                                        return;
                                    if (console.log("xhr.status: " + l.status), 200 !== l.status)
                                        throw new Error("Failed with status " + l.status);
                                    postResult({
                                        content: l.response,
                                        contentType: l.getResponseHeader("Content-Type"),
                                        size: l.getResponseHeader("Content-Length")
                                    }),
                                    e()
                                } catch (e) {
                                    s(e)
                            ...
                            console.log("config url: " + a.MuleSoftBaseURL);
                            var o = a.MuleSoftBaseURL + t;
                            console.log("file download url: " + o);
                            var i = new FormData;
                            i.append("attributes", JSON.stringify({
                                    name: n.fileName,
                                    parent: {
                                        id: "0"
                                    }
                                }));
                            let r = n.content.content;
                            var l;
                            i.append("file", r),
                            (l = new XMLHttpRequest).onreadystatechange = function () {
                                if (4 === l.readyState) {
                                    if (201 !== l.status)
                                        throw new Error("Failed with status " + JSON.stringify(l.response));
                                    postResult({
                                        result: n.content
                                    })
                                }
                            ...
                            l.open("POST", o),
                            l.withCredentials = !0,
                            l.setRequestHeader("destination_s3", "ATP"),
                            l.send(i)
                        })
                    }
                    (0, a, o);
                ...
                case "delete":
                    ...
                    break;
                default:
                    throw new Error("The method " + n + " is not supported.")
                }
            }
            (a, o, 0, i);
            break;
        default:
            throw new Error("The object " + n + " is not supported.")
        }
    }
